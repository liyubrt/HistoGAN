...
def process_image(model, name, input_image, target_hist, image_size=256,
  ...
  img = Image.open(input_image)  # 512x1024, max 255
  original_img = np.array(img) / 255  # np array, 512x1024x3, max 1.0

  # generate reference for pyramid upsampling
  transform = transforms.Compose([transforms.ToTensor()])
  reference = torch.unsqueeze(transform(img), dim=0).to(
    device=torch.cuda.current_device())  # tensor, 1x3x512x1024, 1.0, float32
  ...
  if width != image_size or height != image_size:
    img = img.resize((image_size, image_size))  # 256x256, max 255
  ...
  img = torch.unsqueeze(transform(img), dim=0).to(
    device=torch.cuda.current_device())  # 1x3x256x256, max 1.0, float32
    ...
    for f in files:
      ...
      if extension == '.npy':
        ...
        h = torch.from_numpy(hist).to(device=torch.cuda.current_device())
          ...
          extension) == '.png'):
        img_hist = Image.open(f)  # 640x512
        img_hist = torch.unsqueeze(transform(img_hist), dim=0).to(
          device=torch.cuda.current_device())  # 1x3x512x640, 1.0
        h = histblock(img_hist)  # 1x3x64x64, 1.0
      else:
        print(f'Warning: File extension of {f} is not supported.')
        continue
      with torch.no_grad():
        samples_name = (f'{os.path.basename(os.path.splitext(input_image)[0])}-'
                        f'{os.path.basename(os.path.splitext(f)[0])}')
        generated_images = model.evaluate(samples_name, image_batch=img,
                        hist_batch=h,
                        resizing=resizing_mode,
                        resizing_method=upsampling_method,
                        swapping_levels=swapping_levels,
      ...
      generated_images = upsampling.pyramid_upsampling(
        generated_images, reference, levels=pyramid_levels,
        swapping_levels=swapping_levels, blending=level_blending)
      torchvision.utils.save_image(generated_images, os.path.join(results_dir, name, samples_name+'.jpg'), nrow=1)
...
class Inferencer:
  def __init__(self, args) -> None:
    ...
    self.model.load(args.load_from)
    ...
    self.histblock = RGBuvHistBlock(insz=args.hist_insz, h=args.hist_bin,
                                    resizing=args.hist_resizing, method=args.hist_method,
                                    sigma=args.hist_sigma,
                                    device=self.device)
...
if __name__ == "__main__":
  ...
  image = (np.array(Image.open(args.input_image)) / 255.).astype(np.float32)
  style = (np.array(Image.open(args.target_hist)) / 255.).astype(np.float32)
  inferencer.process_image(image, style)
